...
will check every selector to see if it matches the element. To do that the browser starts 
...
without having to add extra classes or elements. It is highly recommended to prefix these selectors with two semicolons,
this to avoid confusing them with the Pseudo Class Selectors.

- `::after`
	
	This selector allows you to target the part after your element. This can be very usefull when you want to append something 
	to every instance of something.
...
- `::before`
	...
	This selector allows you to target the part before your element. This can be very usefull when you want to prepend something 
	to every instance of something.
...
Selects all elements with the given ID. In an ideal situation this is only one element. The selector is prefixed with a hashtag.
...
In contrary to the pseudo element selectors, there are tons of pseudo class selectors. These selectors
allow you to select an element with a specific state rather than a part of an element. It is highly recommended 
to prefix these selectors with only one semicolon.
...
- `:checked`
	...
	Selects an element with the **checked** state. The selector only works with input elements (only for checkboxes and radiobuttons) or option elements.
...
- `:invalid`
	...
	#### Example
	```html 
	<input type="email" value="joske@vermeulen@euri.com.be"/>
	```
	```css
	input:invalid {
	  background-color: #bada55;
